...
for vm in vmset.findall('vm'):
    ...
    for i in range(1, numClones + 1):
        ...
        if vrdpEnabled and vrdpEnabled == 'true':
            ...
            fullPath = os.path.join(baseGroupname,"RDP")
            create_rdp_file(fullPath,newvmName+"_"+str(vrdpBaseport)+".rdp", ipAddress, vrdpBaseport)
            create_rdesktop_file(fullPath,newvmName+"_"+str(vrdpBaseport)+".sh", ipAddress, vrdpBaseport)
